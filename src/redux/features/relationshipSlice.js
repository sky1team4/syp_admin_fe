...
export const fetchRelationships = createAsyncThunk('relationships/fetchAll', async () => {
  ...
  const response = await axios.get('http://localhost:3000/relationships/findAll', {
    headers: {
...
export const saveRelationship = createAsyncThunk('relationships/save', async (relationship) => {
  ...
  const response = await axios.post('http://localhost:3000/relationships/create', relationshipData, {
    headers: {
      Authorization: `Bearer ${token}`,
    },
  });
...
export const updateRelationship = createAsyncThunk('relationships/update', async ({ id, data }) => {
  const token = getToken();
  const response = await axios.put(`http://localhost:3000/relationships/${id}`, data, {
    headers: {
      Authorization: `Bearer ${token}`,
    },
  });
